...
#include <shapofont/ShapoSansDigitP_s16c14w2_hs.hpp>
#include <shapofont/ShapoSansP_s11c09w2a1_hs.hpp>
...
static void drawString(const mf::Font& font, const char* str, int16_t x,
                       int8_t row) {
  int8_t width, xAdvance;
  int8_t rows = (font.fontHeight() + 7) / 8;
  for (const char* c = str; *c; c++) {
    mf::drawChar(font, *c, glyphBuff, &width, &xAdvance);
    display.drawImage(glyphBuff.data, x, row, MAX_GLYPH_WIDTH, width, rows);
    uint8_t spacing = xAdvance - width;
    display.fillRect(x + width, row, spacing, rows, 0x00);
    x += xAdvance;
  }
}
...
int main() {
  ...
  while (1) {
    drawString(ShapoSansP_s11c09w2a1_hs, text1, x1, 0);
    x1 -= 1;
    if (x1 < -300) x1 = DISP_W;

    drawString(ShapoSansP_s11c09w2a1_hs, text2, x2, 2);
    x2 -= 1;
    if (x2 < -300) x2 = DISP_W;

    drawNumber(ShapoSansDigitP_s16c14w2_hs, number++, 0, 5);
    _delay_ms(1);
  }
}
